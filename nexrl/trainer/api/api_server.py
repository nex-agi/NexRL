...
# ============================================================================
# OPERATION QUEUE SYSTEM - Greedy Processing
# ============================================================================
#
# Purpose: Process operations from all worker groups in FCFS order without
#          GPU exclusivity constraints.
#
# Key Components:
# ---------------
# 1. group_operation_queues: Separate queue for each worker group's operations
# 2. queue_condition: Condition variable for signaling new operations
#
# Workflow:
# ---------
# 1. Each worker group has its own operation queue (FIFO within group)
# 2. Queue processor greedily processes operations from all groups in FCFS order
# 3. No GPU locking - operations execute as soon as they reach the front of any queue
#
# ============================================================================
...
# Per-group operation queues
group_operation_queues: Dict[str, Queue] = {}
queue_processor_thread = None
queue_processor_stop_event = threading.Event()
queue_condition = threading.Condition()  # For signaling when to check for group transitions
...
def process_operation_queue():
    """Background thread to process queued operations greedily (FCFS across all groups)

    Key logic:
    1. Each worker group has its own queue
    2. Process operations from all queues in FCFS order (by enqueue_time)
    3. No GPU locking - operations execute immediately when they reach the front
    """
    global group_operation_queues, queue_processor_stop_event, zmq_coordinators, queue_condition

    logger.info("Starting operation queue processor thread with greedy FCFS processing")

    while not queue_processor_stop_event.is_set():
        try:
            queued_op = None

            # Find oldest waiting operation across all groups (FCFS)
            oldest_op = None
            oldest_group = None
            peeked_ops = []  # List of (group_id, operation) tuples

            # Peek at the front of each queue
            for group_id, group_queue in list(group_operation_queues.items()):
                try:
                    op = group_queue.get_nowait()
                    peeked_ops.append((group_id, op))

                    # Check if this is the oldest so far
                    if oldest_op is None or op.enqueue_time < oldest_op.enqueue_time:
                        oldest_op = op
                        oldest_group = group_id
                except Empty:
                    continue

            # Put back all operations except the oldest one
            for group_id, op in peeked_ops:
                if oldest_op is None or (
                    group_id != oldest_group or op.operation_id != oldest_op.operation_id
                ):
                    group_operation_queues[group_id].put(op)

            if oldest_op:
                queued_op = oldest_op
                wait_time = time.time() - oldest_op.enqueue_time
                logger.info(
                    f"Processing operation '{queued_op.operation_id}' from group '{oldest_group}' (waited {wait_time:.2f}s)"
                )

            # If we got an operation, execute it
            if queued_op:
                logger.info(
                ...
                try:
                    # Get coordinator for this operation (not needed for registration)
                    ...
                    if queued_op.operation_type == "register":
                        ...
                        logger.info(
                            f"Worker group '{queued_op.identifier}' successfully registered"
                        )
                    ...
                    logger.error(
                        f"Failed to process queued operation {queued_op.operation_id}: {e}",
                        exc_info=True,
                    )
                    queued_op.error = str(e)
                    queued_op.result = None

                # Signal completion (from background thread to asyncio event loop)
                if queued_op.result_future and queued_op.event_loop:
                    queued_op.event_loop.call_soon_threadsafe(queued_op.result_future.set)
            else:
                # No operation available, wait a bit
                with queue_condition:
                    queue_condition.wait(timeout=0.5)

        except Exception as e:
            logger.error(f"Error in queue processor thread: {e}", exc_info=True)

    logger.info("Operation queue processor thread stopped")
...
async def submit_queued_operation(
    ...
    # Submit to group's queue
    group_operation_queues[identifier].put(queued_op)
    logger.info(
        f"Queued operation {operation_id} ({operation_type}) for group '{identifier}' (requires_gpu={requires_gpu})"
    )

    # Signal the queue processor to check for new operations
    with queue_condition:
...
async def register_worker_group(request: RegisterWorkerGroupRequest):
    ...
    """
    global zmq_coordinators, heartbeat_lock, heartbeat_timestamps
    ...
    try:
        ...
        # Check if coordinator already exists for this identifier
        if identifier in zmq_coordinators:
            zmq_coordinator = zmq_coordinators[identifier]
            logger.info(f"Worker group '{identifier}' already registered, returning existing info")
        else:
            # Auto-assign base port
            used_ports = set()
            for coord in zmq_coordinators.values():
                used_ports.add(coord.base_port)

            zmq_base_port = 5555
            while zmq_base_port in used_ports:
                zmq_base_port += 100

            # Create new coordinator (but don't add to zmq_coordinators yet)
            dispatch_mode = request.dispatch_mode or "scatter"
            zmq_coordinator = APIZMQCoordinator(
                world_size=request.world_size, base_port=zmq_base_port, dispatch_mode=dispatch_mode
            )

            # Initialize heartbeat tracking for new group
            with heartbeat_lock:
                heartbeat_timestamps[identifier] = time.time()

            # Add to zmq_coordinators
...
async def deregister_worker_group(identifier: str):
    ...
    """
    global zmq_coordinators, heartbeat_timestamps, heartbeat_lock, group_operation_queues
...
async def get_coordinator_attr(attr_name: str, identifier: Optional[str] = None):
    ...
    Used by ActorWorkerClient.__getattr__ to proxy attribute access.
...
def main():
    ...
    # Start operation queue processor thread
    queue_processor_thread = threading.Thread(
        target=process_operation_queue, daemon=True, name="queue-processor"
    )
    queue_processor_thread.start()
    logger.info("Started operation queue processor thread")
    ...
    try:
        ...
        logger.info("Stopping operation queue processor thread...")
        queue_processor_stop_event.set()
