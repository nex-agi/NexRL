...
使用公共镜像（`nexagi/nexrl:v1.4.0`、`lmsysorg/sglang:v0.5.4.post2`）和 `/tmp` 存储 - 非常适合测试！
...
- `NEXRL_WORKER_IMAGE`：Worker 镜像（默认：`nexagi/nexrl:v1.4.0`）
- `NEXRL_CONTROLLER_IMAGE`：Controller 镜像（默认：`nexagi/nexrl:v1.4.0`）
